...
<body class="auth-body">
  ...
  <main class="auth-card">
    ...
    {{-- Tampilkan error dari back()->withErrors() --}}
    @if ($errors->any())
    ...
    <form class="auth-form" id="signinForm" method="POST" action="{{ route('login.post') }}">
      @csrf
      <label class="auth-field">
        <span>Email</span>
        <input type="email" name="email" id="signinEmail" placeholder="nama@email.com" autocomplete="email" value="{{ old('email') }}" required />
      </label>
